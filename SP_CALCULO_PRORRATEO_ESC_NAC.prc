CREATE OR REPLACE PROCEDURE DMESCO.SP_ATRA_CALC_PRORRATEO_ESC_NAC (
...
SELECT A.CLAVE_EJERCICIO_INDICADOR,
      ...
      || '''
       ...
       ''UPD'' as ACCION  
    FROM DMESCO.ATRA_DET_SOLICITANTE A
        JOIN DMESCO.ATRA_REL_MAJOR_ESCUELA B
         ON A.CLAVE_MAJOR_SOLICITANTE = B.CLAVE_MAJOR_PGMA_ACAD
   WHERE A.CLAVE_EJERCICIO_INDICADOR = '''
      || P_CLAVE_EJERCICIO_INDICADOR
      || '''
               AND A.FECHA_INFORMACION = TO_DATE ( '''||FECHA_INFORMACION ||''', ''YYYY-MM-DD'')
      ...
      || P_CORTE
      || '''                        
      AND B.CLAVE_ESCUELA_NACIONAL in (10,11)      
      AND A.CLAVE_MAJOR_PGMA_ACAD IN (''ESC'', ''AMC'')
      AND A.IND_CONSIDERADO_BASE = ''SI''         
GROUP BY A.CLAVE_EJERCICIO_INDICADOR, A.CLAVE_CAMPUS, A.CLAVE_MAJOR_PGMA_ACAD
';
